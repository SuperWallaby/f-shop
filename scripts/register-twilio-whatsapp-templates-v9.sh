...
# Registers WhatsApp templates (v9) in Twilio Content API.
# v9 rules:
# - Put the booking link as a WhatsApp URL button (CTA), not plain text URL.
# - No colon characters ":" in the message body.
# - Keep variables minimal (2 vars: date, time).
...
#
# Usage:
#   bash scripts/register-twilio-whatsapp-templates-v9.sh
    ...
    payload="$(cat <<JSON
{
  ...
  "types": {
    ...
    "twilio/call-to-action": {
      "body": "Reference",
      "actions": [
        { "type": "URL", "title": "Open", "url": "${REFERENCE_URL}" }
      ]
    }
  },
  "variables": ${variables}
...
# ---------------------------------------------------------------------------
# Templates (v9, URL button via twilio/call-to-action, no ":" in body)
...
# 1) Booking confirmed (v9)
post_content "fasea_booking_confirmed_en_v9" "$(cat <<'JSON'
"Fasea Pilates booking confirmed\nDate {{1}}\nTime {{2}}"
JSON
)" "${SAMPLE_VARS}"

# 2) Reminder (v9)
post_content "fasea_booking_reminder_en_v9" "$(cat <<'JSON'
"Fasea Pilates booking reminder\nDate {{1}}\nTime {{2}}"
JSON
)" "${SAMPLE_VARS}"

# 3) Cancelled by client (v9)
post_content "fasea_booking_cancelled_by_client_en_v9" "$(cat <<'JSON'
"Fasea Pilates booking cancelled\nDate {{1}}\nTime {{2}}"
JSON
)" "${SAMPLE_VARS}"

# 4) Cancelled by instructor (v9)
post_content "fasea_class_cancelled_by_instructor_en_v9" "$(cat <<'JSON'
"Fasea Pilates class cancelled\nDate {{1}}\nTime {{2}}"
JSON
)" "${SAMPLE_VARS}"

# 5) No-show (v9)
post_content "fasea_no_show_en_v9" "$(cat <<'JSON'

...
/** Keep studio admin signed in for a long time (same spirit as member sessions). */
const ADMIN_SESSION_MAX_AGE_SEC = 60 * 60 * 24 * 730; // ~2 years
...
export function verifyAdminSessionValue(value: string | undefined): boolean {
  ...
  try {
    ...
    const maxAgeMs = ADMIN_SESSION_MAX_AGE_SEC * 1000;

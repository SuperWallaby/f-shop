...
export default function AdminPage() {
  ...
  const [selectedDay, setSelectedDay] = useState<Date | undefined>(() => {
    ...
    const dt = DateTime.now().setZone(BUSINESS_TIME_ZONE);
  ...
  const [adminItemsSaveMsg, setAdminItemsSaveMsg] = useState<string | null>(
    null,
  );
  ...
  const hasAdminItemsChanges = useMemo(() => {
    ...
    for (const d of adminItemsDraft) {
      ...
      if (
        ...
        (o.autoCancelMinBookings ?? null) !==
          (d.autoCancelMinBookings ?? null) ||
        (o.autoCancelCutoffHours ?? null) !==
          (d.autoCancelCutoffHours ?? null) ||
        exO !== exD ||
        o.active !== d.active
      ) {
        return true;
    ...
    try {
      ...
      for (const d of adminItemsDraft) {
        ...
        if ((o.color ?? "") !== (d.color ?? "")) {
          ...
          if (!normalized)
            patch.randomizeColor = true; // empty/invalid => auto-pick a valid pastel
  ...
  const [loadedWeeklyPatternCount, setLoadedWeeklyPatternCount] = useState<
    number | null
  >(null);
  ...
  const [genFrom, setGenFrom] = useState<string>(() =>
    DateTime.now().setZone(BUSINESS_TIME_ZONE).toFormat("yyyy-LL-dd"),
  );
  ...
  const [genItemId, setGenItemId] = useState<string>("");
  ...
  async function loadSettings() {
    ...
    try {
      ...
      if (!patternItemId) {
        const first =
          adminItems.find((it) => it.active)?.id ?? adminItems[0]?.id;
  ...
  function pastePatternDay(dayKey: string) {
    ...
    if (existing.length > 0) {
      const ok = window.confirm(
        "Replace this weekday pattern with the copied one?",
      );
      if (!ok) return;
    }
  ...
  async function saveSettings() {
    ...
    try {
      if (settingsLoading || !settingsLoaded) {
        throw new Error(
          "Settings not loaded yet. Please click Reload and wait before saving.",
        );
      }
      ...
      const uiCount = Object.values(weeklyPattern).reduce(
        (acc, v) => acc + v.length,
        0,
      );
      const loadedCount =
        typeof loadedWeeklyPatternCount === "number"
          ? loadedWeeklyPatternCount
          : null;
      ...
      const res = await fetch("/api/admin/settings", {
        ...
        body: JSON.stringify({
          businessTimeZone,
          weeklyPattern,
          ...(confirmEmptyWeeklyPattern
            ? { confirmEmptyWeeklyPattern: true }
            : {}),
  ...
  async function saveRulesOnly() {
    ...
    try {
      ...
      if (!res0.ok || !json0?.ok) {
        throw new Error(
          json0?.error?.message ?? "Failed to load current settings",
        );
      }
      ...
      setSaveSettingsMsg(
        e instanceof Error ? e.message : "Failed to save rules",
      );
    } finally {
      setSavingSettings(false);
    }
    ...
    try {
      ...
      if (!res.ok || !json?.ok) {
        if (res.status === 409) {
          ...
          const overlapsTruncated = Boolean(
            json?.error?.details?.overlapsTruncated,
          );
      ...
      <div className="min-h-screen bg-[#FAF8F6] text-[#444444] px-6 py-24">
        ...
        <main className="max-w-xl mx-auto mt-16">
          <div className="bg-white/70 border border-[#E8DDD4] rounded-3xl p-8 shadow-sm">
            ...
            <p className="text-sm text-[#716D64] mb-6">
              ...
              <div className="bg-white/70 border border-[#E8DDD4] rounded-3xl p-6 shadow-sm">
                ...
                <div className="mt-4">
                  ...
                  <div className="mt-2 text-xs text-[#716D64]">
                    Changing this will reload the sessions for the selected
                    date.
                  </div>
                </div>
              </div>
                      ...
                      >
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() =>
                                patchSlot(s.id, { cancelled: !s.cancelled })
                              }
                              className="px-4 py-2 rounded-full border border-[#E8DDD4] bg-white/80 text-sm hover:shadow-sm transition"
                            >
            ...
            <div className="bg-white/70 border border-[#E8DDD4] rounded-3xl p-6 shadow-sm">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <div className="text-xs text-[#716D64] mt-1">
                    Sessions are generated from this pattern, but existing
                    sessions won’t be changed.
                  </div>
                </div>
                <button
                ...
                <div className="mt-6 space-y-6">
                  ...
                  {WEEKDAYS.map((d) => (
                    ...
                    >
                      ...
                      ) : (patternByItemUi[patternItemId]?.[d.key] ?? [])
                          .length === 0 ? (
                        <div className="mt-3 text-sm text-[#716D64]"></div>
                      ) : (
                        <div className="mt-3 space-y-3">
                          {(patternByItemUi[patternItemId]?.[d.key] ?? []).map(
                            (row, idx) => (
                              <div
                                key={idx}
                              ...
                              >
                                ...
                                <button
                                  type="button"
                                  onClick={() =>
                                    removePatternRow(patternItemId, d.key, idx)
                                  }
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          ...
                          <div className="mt-2 text-sm text-[#716D64]">
                            These overlaps are checked within the same class
                            type only.
                          </div>
                        </div>
                        <button
                      ...
                      <div className="mt-5 rounded-3xl border border-[#E8DDD4] bg-white/70 p-5">
                        <div className="flex items-center justify-between gap-4 flex-wrap">
                          <div className="text-sm text-[#444444] font-medium">
                            {generateOverlapModal.overlaps.length} overlap(s)
                            shown
                            {generateOverlapModal.overlapsTruncated
                              ? " (truncated)"
                              : ""}
                          </div>
                          <Switch
                        ...
                        <div className="mt-4 max-h-[45vh] overflow-y-auto rounded-2xl border border-[#E8DDD4] bg-white">
                          <div className="divide-y divide-[#E8DDD4]">
                            {generateOverlapModal.overlaps.map((o, idx) => {
                              const itemName =
                                adminItems.find((it) => it.id === o.itemId)
                                  ?.name ?? o.itemId;
                              return (
          ...
          <section className="bg-white/70 border border-[#E8DDD4] rounded-3xl p-6 shadow-sm">
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div>
                ...
                <div className="text-xs text-[#716D64] mt-1">
                  Capacity is defined on the class type and applied to all its
                  sessions.
                </div>
                {hasAdminItemsChanges ? (
                  <div className="text-xs text-[#A66A4A] mt-2">
                    Unsaved changes
                  </div>
                ) : null}
              </div>
            ...
            {!adminItemsLoading && (
              <div className="mt-6 grid gap-4">
                <div className="rounded-3xl border border-[#E8DDD4] bg-white p-5">
                  <div className="font-serif font-semibold mb-3">
                    Create class type
                  </div>
                  <div className="grid gap-3 sm:grid-cols-2">
                    <label className="grid gap-1">
                    ...
                    <label className="grid gap-1 sm:col-span-2">
                      ...
                      />
                      <div className="text-[11px] text-[#716D64]">
                        If two class types share the same linked key, one
                        booking will block all others at the same date/time.
                      </div>
                    </label>
                    <button
                      type="button"
                          ...
                          >
                            ...
                            <div className="mt-4 grid gap-3 sm:grid-cols-2">
                                    ...
                                    onClick={() => {
                                      setAdminItemsDraft((prev) => {
                                        const other = prev
                                          .filter((x) => x.id !== it.id)
                                          .map((x) => x.color ?? "");
                                        const next = pickUnusedPastelRandom(
                                          other,
                                          {
                                            avoid: it.color ?? "",
                                          },
                              ...
                              <label className="grid gap-1">
                                ...
                                />
                                <div className="text-[11px] text-[#716D64]">
                                  Same key = mutually exclusive at the same
                                  date/time.
                                </div>
                              </label>
                              <label className="grid gap-1 sm:col-span-2">
                              ...
                              <details className="sm:col-span-2 rounded-2xl border border-[#E8DDD4] bg-white/70 px-4 py-3">
                                ...
                                <div className="mt-3 grid gap-3 sm:grid-cols-2">
                                  ...
                                  <div className="text-[11px] text-[#716D64] sm:col-span-2">
                                    If enabled, sessions will be automatically
                                    cancelled if the number of bookings is below
                                    the minimum at the cutoff time before start.
                                    A Twilio WhatsApp message will be sent.
                                  </div>
                                  <label className="grid gap-1">
                                    <span className="text-xs text-[#716D64]">
                                      Minimum bookings
                                        ...
                                        setAdminItemsDraft((prev) =>
                                          prev.map((x) =>
                                            x.id === it.id
                                              ? {
                                                  ...x,
                                                  autoCancelMinBookings:
                                                    Number.isFinite(v) ? v : 2,
                                                }
                                              : x,
                                          ),
                                        );
          ...
          <section className="bg-white/70 border border-[#E8DDD4] rounded-3xl p-6 shadow-sm">
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              <label className="grid gap-1">
                ...
                />
                <div className="text-[11px] text-[#716D64]">
                  Example: 24 means users must book at least 24 hours before
                  session start.
                </div>
              </label>
